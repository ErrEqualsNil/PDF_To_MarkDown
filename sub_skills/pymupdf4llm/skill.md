...
Requires `llm_api_key` and `llm_base_url` and `llm_model` in `sub_skills/pymupdf4llm/config.json`:
...
FallbackParser().parse(Path("<pdf_path>"), Path("<out_dir>"))
# Output: <out_dir>/<original_pdf_name>.md
```
